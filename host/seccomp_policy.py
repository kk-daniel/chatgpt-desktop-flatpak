...
def build_program():
    ...
    #   6  nr == socket ? -> 7 : -> 9+n
    #   7  load arg0 (family)
    #   8+k  family == allowed[k] ? -> 9+n : next
    #   8+n  return EAFNOSUPPORT
    #   9+n  load nr        <- rejoin here, socket falls through the blocklist
    resume = 9 + n
